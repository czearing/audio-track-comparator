/// MTT MusiCNN instrument/auto-tagger.
///
/// Model: instrument_detector.onnx  (mtt-musicnn-1.onnx from Essentia)
///   Input:  [n_patches, 187, 96]  float32   (n_patches × time_frames × mel_bins)
///   Output: [n_patches, 50]       float32   (sigmoid tag probabilities)
///
...
///   n_mels      = 96
///   patch_size  = 187 frames   (one model input)
///   patch_hop   = 93  frames   (50% overlap between adjacent patches)
use crate::resample;
use anyhow::Context;
use ndarray::Array3;
...
const INSTRUMENT_SAMPLE_RATE: u32 = 16_000;
const FRAME_SIZE: usize = 512;
const HOP_SIZE: usize = 256;
const N_MELS: usize = 96;
const PATCH_SIZE: usize = 187; // time frames per patch
const PATCH_HOP: usize = 93; // 50% overlap
...
fn build_mel_filterbank() -> Vec<Vec<f32>> {
    ...
    let sr = INSTRUMENT_SAMPLE_RATE as f64;
...
/// LogC compression matching MusiCNN training preprocessing:
...
/// Uses PATCH_HOP-frame hop between adjacent patches (50% overlap).
fn compute_patches(mono_16k: &[f32]) -> Vec<Vec<f32>> {
    ...
    for frame_idx in 0..n_frames {
        ...
        // Project to mel and apply logC compression
    ...
    // Split into overlapping patches of PATCH_SIZE frames, hop PATCH_HOP
...
/// Detect top-5 instrument tags from mono 22050Hz audio using the MTT MusiCNN model.
///
...
pub fn detect(mono_22050: &[f32], cache_dir: &Path) -> anyhow::Result<Vec<String>> {
    ...
    let labels_json = std::fs::read_to_string(&labels_path)
        .context("Failed to read instrument_labels.json")?;
    let labels: Vec<String> =
        serde_json::from_str(&labels_json).context("Failed to parse instrument_labels.json")?;
    if labels.len() != 50 {
        anyhow::bail!(
            "instrument_labels.json: expected 50 labels, got {}",
            labels.len()
        );
    }

    // Resample 22050 → 16000 Hz
    let mono_16k = resample::resample(mono_22050, 22050, INSTRUMENT_SAMPLE_RATE)
        .context("Failed to resample to 16kHz for instrument model")?;
    ...
    // Output 0 is [n_patches, 50] tag probabilities
    ...
    if total_elements != n_patches * 50 {
        anyhow::bail!(
    ...
    let mut avg_scores = vec![0.0f32; 50];
    for p in 0..n_patches {
        for g in 0..50 {
            avg_scores[g] += raw[p * 50 + g];
        }
    }
